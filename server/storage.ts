...
export function createStorage(dataDir: string = DEFAULT_DATA_DIR): Storage {
  ...
  if (supabaseUrl && supabaseKey) {
    // Try to use Supabase but wrap in an adaptive storage that falls back to file storage
    return new (class AdaptiveStorage implements Storage {
      private backend: Storage;

      constructor() {
        this.backend = new SupabaseStorage(supabaseUrl, supabaseKey);
      }

      private async tryFallback(err: any, methodName: string) {
        console.error(`AdaptiveStorage: switching to FileStorage due to ${methodName} error:`, err?.message || err);
        // replace backend with FileStorage
      ...
      async readJson<T>(key: string, fallback: T): Promise<T> {
        try {
          return await this.backend.readJson(key, fallback);
        } catch (err) {
          const file = await this.tryFallback(err, 'readJson');
          return await file.readJson(key, fallback);
        }
      }

      async writeJson(key: string, data: unknown): Promise<void> {
        try {
          return await this.backend.writeJson(key, data);
        } catch (err) {
          const file = await this.tryFallback(err, 'writeJson');
          return await file.writeJson(key, data);
        }
      }

      async hasApk(projectId: string): Promise<boolean> {
        try {
          return await this.backend.hasApk(projectId);
        } catch (err) {
          const file = await this.tryFallback(err, 'hasApk');
          return await file.hasApk(projectId);
        }
      }

      async getApk(projectId: string): Promise<{ data: Buffer; meta: ApkMeta } | null> {
        try {
          return await this.backend.getApk(projectId);
        } catch (err) {
          const file = await this.tryFallback(err, 'getApk');
          return await file.getApk(projectId);
        }
      }

      async putApk(projectId: string, data: Buffer, meta: ApkMeta): Promise<void> {
        try {
          return await this.backend.putApk(projectId, data, meta);
        } catch (err) {
          const file = await this.tryFallback(err, 'putApk');
          return await file.putApk(projectId, data, meta);
        }
      }

      async deleteApk(projectId: string): Promise<void> {
        try {
          return await this.backend.deleteApk(projectId);
        } catch (err) {
          const file = await this.tryFallback(err, 'deleteApk');
          return await file.deleteApk(projectId);
        }
      }

      async getApkMeta(projectId: string): Promise<ApkMeta | null> {
        try {
          // Optional on supabase; may throw if backend doesn't implement
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          return (this.backend as any).getApkMeta ? await (this.backend as any).getApkMeta(projectId) : null;
        } catch (err) {
          const file = await this.tryFallback(err, 'getApkMeta');
          return await file.getApkMeta(projectId);
        }
      }
      ...
      async writeApkMeta(projectId: string, meta: ApkMeta): Promise<void> {
        try {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          if ((this.backend as any).writeApkMeta) return await (this.backend as any).writeApkMeta(projectId, meta);
          const file = new FileStorage(dataDir);
          return await file.writeApkMeta(projectId, meta);
        } catch (err) {
          const file = await this.tryFallback(err, 'writeApkMeta');
          return await file.writeApkMeta(projectId, meta);
        }
      }
      ...
      async createApkDownloadUrl(projectId: string): Promise<string | null> {
        try {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          return (this.backend as any).createApkDownloadUrl ? await (this.backend as any).createApkDownloadUrl(projectId) : null;
        } catch (err) {
          const file = await this.tryFallback(err, 'createApkDownloadUrl');
          // Local files: no signed url
          return null;
        }
      }
      ...
      async createApkUploadUrl(projectId: string): Promise<{ uploadUrl: string; path: string } | null> {
        try {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          return (this.backend as any).createApkUploadUrl ? await (this.backend as any).createApkUploadUrl(projectId) : null;
        } catch (err) {
          const file = await this.tryFallback(err, 'createApkUploadUrl');
          return null;
        }
      }
    })();
